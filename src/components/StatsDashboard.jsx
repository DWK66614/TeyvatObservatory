...
export default function StatsDashboard({ characters }) {
  ...
  const avgCV = Math.round(characters.reduce((s, ch) => s + (ch.critValue || 0), 0) / characters.length)
  const ssCount = characters.filter(ch => (ch.critValue || 0) >= 200).length
  ...
  return (
    <div className="space-y-6">
      <div className="card p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-5">
          <div className="text-right">
            <div className="text-[10px] font-medium tracking-wide" style={{ color: c.textFaint }}>
              SS 级以上
            </div>
          ...
          <div className="text-right">
            ...
            <div className="text-3xl font-bold font-mono" style={{ color: c.sage }}>
              {characters.length}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-0.5">
          {[...characters]
            ...
            .map((ch, i) => {
              ...
              const maxCV = Math.max(...characters.map(c2 => c2.critValue || 0), 1)
              const grade = getCVGrade(cv, c)
              return (
                <div key={ch.id} className="flex items-center gap-3 py-2.5 group rounded-lg px-2 -mx-2 transition-colors"
                  ...
                  <span className="w-6 text-xs font-mono text-right flex-shrink-0" style={{
                    color: i === 0 ? c.gold : i < 3 ? c.textMuted : c.textFaint,
                  }}>
                    {i === 0 ? '✧' : i + 1}
                  </span>
                  <div className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                       style={{ background: el?.color || c.textFaint }} />
                  <span className="w-20 text-xs truncate flex-shrink-0" style={{ color: c.textSecondary }}>
                    {ci?.name || `#${ch.id}`}
                  </span>
              ...
              {[...characters].sort((a, b) => (b.level || 0) - (a.level || 0)).map(ch => {
                ...
                return (
                  <tr key={ch.id} className="transition-colors"
                      ...
                      onMouseEnter={e => e.currentTarget.style.background = c.surfaceSubtle}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                    <td className="py-3 pl-6 pr-3 sticky left-0 z-10" style={{ background: c.surface }}>
                      <div className="flex items-center gap-2.5">
                        <div className="w-1.5 h-5 rounded-full flex-shrink-0" style={{ background: el?.color || c.textFaint }} />
                        <span className="truncate font-medium" style={{ color: c.textSecondary }}>
                          {ci?.name || `#${ch.id}`}
                        </span>
                      </div>
                    </td>
                    <td className="text-right py-3 px-2 font-mono" style={{ color: c.textMuted }}>{ch.level}</td>
                    <td className="text-right py-3 px-2 font-mono" style={{ color: c.textSecondary }}>{fmt(s.hp)}</td>
                    <td className="text-right py-3 px-2 font-mono" style={{ color: c.textSecondary }}>{fmt(s.atk)}</td>
                    <td className="text-right py-3 px-2 font-mono" style={{ color: c.textSecondary }}>{fmt(s.def)}</td>
                    <td className="text-right py-3 px-2 font-mono" style={{ color: c.textSecondary }}>{fmt(s.em)}</td>
                    ...
                    <td className="text-right py-3 px-2 font-mono" style={{ color: c.textSecondary }}>
      ...
      {/* Artifact Summary */}
      {(() => {
        const total = characters.reduce((s, ch) => s + (ch.artifacts?.length || 0), 0)
        const five = characters.reduce((s, ch) => s + (ch.artifacts?.filter(a => a.rarity >= 5).length || 0), 0)
        return (
          <div className="card p-6">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-5 rounded-xl text-center"
                ...
                <div className="text-xs mt-1.5 font-medium" style={{ color: c.textMuted }}>
                  圣遗物总数
                </div>
              </div>
              <div className="p-5 rounded-xl text-center"
