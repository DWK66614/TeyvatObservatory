...
export default function SpiralAbyss({ playerInfo, characters }) {
  ...
  const stats = [
    ...
    { label: '成就完成', value: playerInfo?.finishAchievementNum || 0, sub: '个', icon: <Trophy className="w-4 h-4" />, color: '#edd9a3' },
    { label: '冒险等阶', value: playerInfo?.level || 0, sub: `世界 ${playerInfo?.worldLevel || 0}`, icon: <Star className="w-4 h-4" />, color: '#b07cf7' },
  ]
