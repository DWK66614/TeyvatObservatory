...
// Generate a constellation chart — stars with connecting lines
function generateConstellation() {
  const stars = []
  const count = 80

  // Create star clusters (constellations) in 3-4 regions
  const clusters = [
    { cx: 15, cy: 20, spread: 18, count: 12 },   // upper-left
    { cx: 75, cy: 15, spread: 16, count: 10 },    // upper-right
    { cx: 20, cy: 70, spread: 15, count: 10 },    // lower-left
    { cx: 80, cy: 65, spread: 18, count: 12 },    // lower-right
    { cx: 50, cy: 40, spread: 12, count: 8 },     // center
  ]
  ...
  clusters.forEach(cluster => {
    for (let i = 0; i < cluster.count; i++) {
      ...
      const size = Math.random() < 0.15
        ? Math.random() * 1.2 + 0.8  // bright named stars
  ...
  // Generate constellation lines between nearby stars in the same cluster
  const lines = []
  for (let ci = 0; ci < clusters.length; ci++) {
    ...
    for (let i = 0; i < clusterStars.length; i++) {
      for (let j = i + 1; j < clusterStars.length; j++) {
        ...
        // Only connect stars within 12% distance, and only some pairs
...
export default function StarBackground() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0" style={{ background: '#080c1d' }}>
      {/* Subtle nebula gradient blobs */}
      <div className="absolute" style={{
        width: '60vw', height: '60vw', maxWidth: 800, maxHeight: 800,
        top: '-25%', right: '-15%',
        background: 'radial-gradient(ellipse, rgba(112,149,196,0.04) 0%, transparent 70%)',
        borderRadius: '50%',
      }} />
      <div className="absolute" style={{
        width: '50vw', height: '50vw', maxWidth: 600, maxHeight: 600,
        bottom: '-20%', left: '-10%',
        background: 'radial-gradient(ellipse, rgba(237,217,163,0.03) 0%, transparent 70%)',
        borderRadius: '50%',
      }} />
      ...
      <div ref={containerRef} className="absolute inset-0 transition-transform duration-[2000ms] ease-out"
        ...
        {stars.map(s => (
          <div
            ...
            style={{
              ...
              width: s.size,
              height: s.size,
              background: s.size > 1 ? '#edd9a3' : '#7095c4',
              opacity: s.opacity,
              boxShadow: s.size > 1
                ? `0 0 ${s.size * 3}px rgba(237,217,163,0.3), 0 0 ${s.size * 6}px rgba(237,217,163,0.1)`
                : 'none',
        ...
        {/* Constellation lines rendered as pseudo-elements via thin divs */}
        {lines.map(l => {
          const dx = l.x2 - l.x1
          const dy = l.y2 - l.y1
          ...
          return (
            <div
              ...
              style={{
                ...
                width: `${length}%`,
                height: '0.5px',
                background: `rgba(112,149,196,${l.opacity * 10})`,
