...
function StatRow({ label, value, max, color = '#edd9a3' }) {
  ...
  return (
    <div className="flex items-center gap-2.5 text-[11px]">
      <span className="w-14 truncate" style={{ color: 'var(--dust)' }}>{label}</span>
      ...
      <span className="w-12 text-right font-mono text-[11px]" style={{ color: 'var(--text)' }}>{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="card-clickable transition-all duration-300" style={{
      ...
      <div className="p-4" onClick={onToggle}>
        <div className="flex items-center gap-3.5">
          <div className="relative flex-shrink-0">
            <div className="w-14 h-14 rounded-xl flex items-center justify-center overflow-hidden"
                 style={{
                   background: element ? `${element.color}12` : c.goldBg,
                   border: `1px solid ${element ? element.color + '20' : c.goldBorder}`,
                 }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              <span className="text-[11px] font-mono" style={{ color: c.textSecondary }}>
                Lv.{character.level}
              </span>
              {character.constellations > 0 && (
                <span className="inline-flex items-center gap-0.5 text-[10px] font-mono font-bold px-1.5 py-0.5 rounded"
                      style={{
                        background: `linear-gradient(135deg, ${c.goldBg}, ${c.goldDim}20)`,
                        color: c.gold,
                        border: `1px solid ${c.goldBorder}`,
                      }}>
                  <span style={{ fontSize: '8px' }}>★</span>
                  {character.constellations}命
                </span>
              )}
            </div>
          </div>
        ...
        {character.talentLevels?.length > 0 && (
          <div className="flex items-center gap-3 mt-3">
            <div className="flex items-center gap-1.5">
              {character.talentLevels.map((t, i) => (
                <span key={i} className="inline-flex items-center gap-0.5 text-[10px] font-mono px-1.5 py-0.5 rounded"
                      style={{ background: c.goldBg, color: c.textSecondary }}>
                  <span style={{ color: c.text }}>{t.label}</span>
                  <span style={{ color: c.gold }}>{t.level}</span>
                  {t.extra > 0 && (
                    <span style={{ color: c.sage, fontSize: '8px' }}>+{t.extra}</span>
                  )}
                </span>
              ))}
            </div>
        ...
        {character.friendship > 0 && (
          <div className="flex items-center gap-1 mt-2.5 text-[10px]"
               style={{ color: character.friendship >= 10 ? c.rust : c.textSecondary }}>
            <Heart className="w-3 h-3" style={{
              fill: character.friendship >= 10 ? c.rust : 'transparent',
              color: character.friendship >= 10 ? c.rust : c.textSecondary,
            }} />
            <span className="font-mono">{character.friendship}</span>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-4">
            <div>
              ...
              <div className="space-y-2.5">
                ...
                <StatRow label="防御力" value={Math.round(character.stats?.def || 0)} max={2000} color={c.nebula} />
                <StatRow label="元素精通" value={character.stats?.em || 0} max={500} color={c.sage} />
                    ...
                    {character.artifacts.map((art, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-2 p-2 rounded-lg"
                             style={{ background: c.surfaceSecondary }}>
                          {art.iconUrl && (
